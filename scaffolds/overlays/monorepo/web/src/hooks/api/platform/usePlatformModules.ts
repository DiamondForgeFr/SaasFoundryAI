...
export const usePlatformModules = (enabled: boolean = true) =>
  useQuery({
    ...
    queryFn: async () => {
      const raw = await apiClientMutator({ url: '/api/accounts/platform/modules', method: 'GET' })
...
export const useTogglePlatformModule = () => {
  ...
  const mutation = useMutation({
    mutationFn: async ({ moduleId, isActive }: { moduleId: number; isActive: boolean }) => {
      return apiClientMutator<{ id: number; name: string; isActive: boolean }>({
        url: `/api/accounts/platform/modules/${moduleId}`,
        method: 'PATCH',
        data: { isActive }
      })
    },
...
export const useToggleSubModule = () => {
  ...
  const mutation = useMutation({
    mutationFn: async ({ moduleId, subModuleId, isActive }: { moduleId: number; subModuleId: number; isActive: boolean }) => {
      return apiClientMutator<{ id: number; name: string; isActive: boolean }>({
        url: `/api/accounts/platform/modules/${moduleId}/sub-modules/${subModuleId}`,
        method: 'PATCH',
        data: { isActive }
      })
    },
