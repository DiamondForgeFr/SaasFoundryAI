...
program
  ...
  .option('--non-interactive', 'Fail if any required value is missing instead of prompting')
  .option('--dry-run', 'Preview changes without writing files or running installers')
  .option('--json', 'Emit one versioned JSON object on stdout (requires --dry-run)')
  .option('--target-profile <profile>', 'Add missing capabilities to reach the target profile (V1: full)')
  ...
  .option('--add-modules <modules>', 'Comma-separated list of modules to add (email, storage, analytics, pwa, harness, srs, sf-skill-context7, sf-skill-atlassian, sf-skill-notion, sf-skill-figma)')
  .option('--workflow <preset>', 'Workflow preset used when adding the harness: solo, saasfoundry, or none')
  .option('--no-workflow', 'Install the harness without workflow configuration')
  .option('--adopt-legacy', 'Adopt a recognized SaaSFoundry project created before manifest support')
  .option('--adopt-plan <fingerprint>', 'Apply the exact legacy-adoption plan reported by a prior dry run')
  .option('--project-name <name>', 'Original project name used to verify a legacy layout')
  .option('--main-branch <branch>', 'Main branch used when legacy API and web repositories cannot prove it')
  .option('--project-description <description>', 'Project description used when adopting the technical stack')
  .option('--structure <structure>', 'Technical stack structure: monorepo or multirepo')
  .option('--db-setup <setup>', 'Database setup for technical stack adoption: docker, credentials, or manual')
  .option('--db-type <type>', 'Database type: postgresql or sql')
  .option('--db-host <host>', 'Database host')
  .option('--db-port <port>', 'Database port (default: first free from 5435)')
  .option('--db-user <user>', 'Database user')
  .option('--db-password <password>', 'Database password')
  .option('--db-name <name>', 'Database name')
  .option('--api-port <port>', 'Host port the adopted API listens on (default: first free from 3500)')
  .option('--web-port <port>', 'Host port the adopted web app listens on (default: first free from 5173)')
  .option('--email-service <service>', 'Email service for technical stack adoption: none or mailersend')
  ...
  .option('--s3-bucket <bucket>', 'S3 bucket name')
  .option('--s3-region <region>', 'S3 region')
  .option('--analytics', 'Include the analytics module in the adopted technical stack')
  .option('--no-analytics', 'Skip the analytics module in the adopted technical stack')
  .option('--pwa', 'Include the installable-app module in the adopted technical stack (default)')
  .option('--no-pwa', 'Skip the installable-app module in the adopted technical stack')
  .option('--context7-api-key <key>', 'Context7 API key (unused — free public API)')
  .option('--atlassian-email <email>', 'Atlassian account email')
  .option('--atlassian-api-token <token>', 'Atlassian API token')
  .option('--atlassian-site <site>', 'Atlassian site name (e.g. "mycompany")')
