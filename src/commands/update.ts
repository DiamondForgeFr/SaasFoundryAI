...
import { SaaSFoundryManifest, SrsToolConfig, isScaffoldManifest } from '../types'
...
export async function updateCommand(opts: UpdateCommandOptions = {}) {
  ...
  try {
    ...
    // for non-scaffold manifests; this narrows the type.
    if (selectedModules.some((m) => m !== 'srs') && !isScaffoldManifest(manifest)) {
      throw new Error('Non-SRS modules require a scaffolded SaaS project (generated by sf new)')
    }
